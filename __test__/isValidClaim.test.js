const isValidClaim = require('../src/isValidClaim');

describe('isValidClaim Tests', () => {
  it('Should validate a claim path against UCA definitions and VC definitions and succeed', (done) => {
    ...
    const validation = isValidClaim(claim, uca, credential);
  ...
  it('Should fail validation of a wrong defined claim path', (done) => {
    const target = () => {
      isValidClaim('first', 'claim-civ:Identity:name-1', 'civ:Credential:GenericId');
    };
  ...
  it('Should fail validation of a wrong defined uca global identifier', (done) => {
    const target = () => {
      isValidClaim('name.first', 'claim-civ:Identity:error-1', 'civ:Credential:GenericId');
    };
  ...
  it('Should fail validation of a wrong defined claim path identifier', (done) => {
    const target = () => {
      isValidClaim('name.error', 'claim-civ:Identity:name-1', 'civ:Credential:GenericId');
    };
  ...
  it('Should fail validation of a wrong defined credential parent identifier', (done) => {
    const target = () => {
      isValidClaim('name.first', 'claim-civ:Identity:name-1', 'civ:Credential:Generic');
    };
