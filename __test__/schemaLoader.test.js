...
describe("schema loading tests", () => {
  ...
  it("test claim definition creation", async () => {
    ...
    expect(claimDefinitions).toEqual(
    ...
    expect(schemaLoader.summaryMap).toEqual(
      expect.objectContaining({
        "identity.credential": {
          ...
          credentials: ["credential-cvc:Identity-v3"],
          labelFor: ["credential-cvc:Identity-v3"],
          changeable: true,
          claimPath: null,
        },
      }),
  ...
  it("correctly loads an array type", async () => {
    ...
    expect(schemaLoader.definitions).toEqual(
      expect.arrayContaining([expect.objectContaining(definition)]),
    );
  });
});
