...
describe("Unit tests for MiniCryptoManager", () => {
  ...
  test("Should sign with XPVT key and verify with XPUB pair", async () => {
    const nonce = new Date().getTime();
    const stringWithNonce = `SomeStringWithNonce${nonce}`;
    const hexHashNonce = sjcl.codec.hex.fromBits(
      sjcl.hash.sha256.hash(stringWithNonce),
    );
    ...
    const hexHashNonceToVerify = sjcl.codec.hex.fromBits(
      sjcl.hash.sha256.hash(stringWithNonceToVerify),
    );

    const keyNameVerify = "KeyNameToVerify";
    cryptoManagerImpl.installKey(keyNameVerify, XPUB);

    const verify = cryptoManagerImpl.verify(
      keyNameVerify,
      hexHashNonceToVerify,
      hexSignature,
    );
