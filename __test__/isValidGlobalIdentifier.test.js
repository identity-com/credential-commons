...
describe("isGlobalIdentifier Tests", () => {
  ...
  test("name-v1 is malformed", () =>
    expect(isGlobalIdentifier("name-v1")).rejects.toThrow(
      /Malformed Global Identifier/,
    ));
  ...
  test("claim-cvc:Name.givenNames-v1 is valid", async () => {
    expect(
      await isGlobalIdentifier("claim-cvc:Name.givenNames-v1"),
    ).toBeTruthy();
  });
  ...
  test("credential-cvc:IdDocument-v1 is valid", async () => {
    expect(
      await isGlobalIdentifier("credential-cvc:IdDocument-v1"),
    ).toBeTruthy();
  });
});
