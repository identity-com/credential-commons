...
describe("Claim Constructions tests", () => {
  ...
  test("Claim construction should fails", async () =>
    expect(Claim.create("name.first", "joao")).rejects.toThrow(
  ...
  test("Claim dont construct incomplete objects", () => {
    ...
    return expect(Claim.create(identifier, value)).rejects.toThrow(
      /Missing required fields to claim-cvc:Identity.name-v1/,
    );
  });
  ...
  test("Claim dont construct invalid day", () => {
    ...
    return Claim.create(identifier, value).catch((e) =>
  ...
  test("Claim should construct a complex Attestatble Value: claim-cvc:Identity.name-v1", async () => {
    const aComplexAttestableValue =
      "urn:name.familyNames:c443e0a97a2df34573f910927e25c58e597e211152dfb650e6210facacc1a065:Santos|urn:name.givenNames:f14ab211784a3b3d2f20d423847a775ad56c3be8104a51aa084f0c94756d953b:Joao|urn:name.otherNames:09a31dab0a537ac5330a07df63effd9d2f55e91845956b58119843835f7dd9ed:Barbosa|";
    const v = await Claim.create("claim-cvc:Identity.name-v1", {
      attestableValue: aComplexAttestableValue,
    });
    expect(v).toBeDefined();
  });
  ...
  test("Claim should return empty array when parsing wrong attestable value", async () => {
    try {
      ...
      const aComplexAttestableValue = {
        attestableValue:
          "buren:name.familyNames:c443e0a97a2df34573f910927e25c58e597e211152dfb650e6210facacc1a065:Santos|buren:name.givenNames:f14ab211784a3b3d2f20d423847a775ad56c3be8104a51aa084f0c94756d953b:Joao|urn:name.otherNames:09a31dab0a537ac5330a07df63effd9d2f55e91845956b58119843835f7dd9ed:Barbosa",
      };
  ...
  test("Construct a claim-cvc:Contact.email-v1 Claim", async () => {
    ...
    const email = await Claim.create(identifier, {
      username: "joao",
      domain: { name: "civic", tld: "com" },
    });
  ...
  test("Should get ALL Claim properties email", async () => {
    const properties = await Claim.getAllProperties(
  ...
  test("Transforming UCA to Claim", async () => {
    ...
    const dateOfBirthUCA = await UserCollectableAttribute.create(
      identifier,
      value,
    );
    ...
    const dateOfBirthClaim = await Claim.create(
      identifier,
      dateOfBirthUCA.getPlainValue(),
    );
    ...
    expect(dateOfBirthClaim.identifier).toBe(
  ...
  test("Transforming alias UCA to Claim", async () => {
    ...
    const evidencesUCA = await UserCollectableAttribute.create(
      identifier,
      value,
    );
    const evidencesAliasUCA = await UserCollectableAttribute.create(
      aliasIdentifier,
      value,
    );

    // converting UCAs to Claims
    const evidencesClaim = await Claim.create(
      identifier,
      evidencesUCA.getPlainValue(),
    );
    const evidencesClaimForAliasUCA = await Claim.create(
      aliasIdentifier,
      evidencesAliasUCA.getPlainValue(),
    );

    // should map to the same claim
    expect(evidencesClaimForAliasUCA.identifier).toEqual(
      evidencesClaim.identifier,
    );
    expect(evidencesClaimForAliasUCA.getPlainValue()).toEqual(
      evidencesClaim.getPlainValue(),
    );
