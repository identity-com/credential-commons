...
describe("Unit tests for Verifiable Credentials", () => {
  ...
  it("Dont construct undefined Credentials", async () => {
    ...
    const dob = await Claim.create(
      "claim-cvc:Identity.dateOfBirth-v1",
      identityDateOfBirth,
    );

    return expect(
      VC.create("cvc:cred:Test", uuidv4(), null, credentialSubject, [
        name,
        dob,
      ]),
  ...
  it(
    "should validate new defined credentials with the obligatory Meta:expirationDate UCA with" +
      " null value",
    async () => {
  ...
  it("New Expirable Credentials", async () => {
    ...
    expect(cred.credentialSubject.identity.dateOfBirth.year).toBe(1978);
    expect(
      _.find(cred.proof.leaves, { identifier: "cvc:Meta:issuer" }),
    ).toBeDefined();
    expect(
      _.find(cred.proof.leaves, { identifier: "cvc:Meta:issuanceDate" }),
    ).toBeDefined();
    expect(cred.expirationDate).toBeDefined();
    expect(
      _.find(cred.proof.leaves, { identifier: "cvc:Meta:expirationDate" }),
    ).toBeDefined();
    expect(cred.proof.leaves).toHaveLength(8);
  });
  ...
  it("New Defined Credentials return the incorrect global Credential Identifier", async () => {
    ...
    expect(cred.getGlobalIdentifier()).toBe(
      "credential-credential-cvc:Identity-v3-3",
    );
  });
  ...
  it("should refresh an temporary anchoring with an permanent one", async () => {
    ...
    cred.requestAnchor = jest.fn().mockImplementation(async () => {
      ...
      const credentialContents = fs.readFileSync(
        "__test__/creds/fixtures/VCPermanentAnchor.json",
        "utf8",
      );
      const mockedVc = await VC.fromJSON(JSON.parse(credentialContents));
      mockedVc.updateAnchor = jest
        .fn()
  ...
  it("Filter claims from Identity Name", async () => {
    const nameUca = await Claim.create(
      "claim-cvc:Identity.name-v1",
      identityName,
    );

    const dobUca = await Claim.create(
      "claim-cvc:Identity.dateOfBirth-v1",
      identityDateOfBirth,
    );
    ...
    expect(
      filtered.credentialSubject.identity.name.otherNames,
    ).not.toBeDefined();
    expect(
      filtered.credentialSubject.identity.name.familyNames,
    ).not.toBeDefined();
  ...
  it("Should filter claims for Email asking for claim-cvc:Contact.email-v1 and return them on the filtered VC", async () => {
    const email = {
      domain: {
        tld: "oVaPsceZ4C",
        name: "UTpHKFyaaB",
    ...
    const emailUca = await Claim.create(
    ...
    expect(filtered.credentialSubject.contact.email.domain.tld).toBe(
      "oVaPsceZ4C",
    );
    expect(filtered.credentialSubject.contact.email.domain.name).toBe(
      "UTpHKFyaaB",
    );
    expect(filtered.credentialSubject.contact.email.username).toBe(
      "ZcMpCBQ0lE",
    );
  });

  it("Should filter claims for Email asking for cvc:Contact:domain and not return the cvc:Contact:address", async () => {
    const email = {
      domain: {
        tld: "oVaPsceZ4C",
        name: "UTpHKFyaaB",
  ...
  it(
    "Should filter claims for Address asking for claim-cvc:Type.address-v1" +
    ...
    async () => {
      ...
      expect(filtered.credentialSubject.identity.address.country).toBe(
        "X2sEB9F9W9",
      );
      expect(filtered.credentialSubject.identity.address.county).toBe(
        "sDlIM4Rjpo",
      );
      expect(filtered.credentialSubject.identity.address.state).toBe(
        "ZZEOrbenrM",
      );
      expect(filtered.credentialSubject.identity.address.street).toBe(
        "JkHgN5gdZ2",
      );
      expect(filtered.credentialSubject.identity.address.unit).toBe(
        "fo9OmPSZNe",
      );
      expect(filtered.credentialSubject.identity.address.city).toBe(
        "LVkRGsKqIf",
      );
      expect(filtered.credentialSubject.identity.address.postalCode).toBe(
        "5JhmWkXBAg",
      );
    },
  );

  it("Should filter claims for PhoneNumber asking for credential-cvc:PhoneNumber-v3 and return the full claim", async () => {
    const value = {
      country: "1ApYikRwDl",
    ...
    expect(filtered.credentialSubject.contact.phoneNumber.country).toBe(
      "1ApYikRwDl",
    );
    expect(filtered.credentialSubject.contact.phoneNumber.countryCode).toBe(
      "U4drpB96Hk",
    );
    expect(filtered.credentialSubject.contact.phoneNumber.extension).toBe(
      "sXZpZJTe4R",
    );
    expect(filtered.credentialSubject.contact.phoneNumber.lineType).toBe(
      "OaguqgUaR7",
    );
    expect(filtered.credentialSubject.contact.phoneNumber.number).toBe(
      "kCTGifTdom",
    );
  });

  it("Should filter claims for GenericDocumentId asking for claim-cvc:Identity.dateOfBirth-v1 and return nothing", async () => {
    const typeValue = "passport";
    const type = await Claim.create(
      "claim-cvc:Document.type-v1",
      typeValue,
      "1",
    );
    const numberValue = "3bj1LUg9yG";
    const number = await Claim.create(
    ...
    const dateOfBirth = await Claim.create(
    ...
    const address = await Claim.create(
    ...
    const properties = await Claim.create(
    ...
    const credential = await VC.create(
      "credential-cvc:GenericDocumentId-v3",
      "",
      null,
      credentialSubject,
      [
  ...
  it(
    "Should filter claims for PhoneNumber asking for cvc:Phone:countryCode and return only the" +
    ...
    async () => {
      ...
      const filtered = credential.filter([
        "claim-cvc:PhoneNumber.countryCode-v1",
      ]);

      expect(filtered.credentialSubject.contact.phoneNumber).toBeDefined();
      expect(
        filtered.credentialSubject.contact.phoneNumber.country,
      ).toBeUndefined();
      expect(filtered.credentialSubject.contact.phoneNumber.countryCode).toBe(
        "U4drpB96Hk",
      );
      expect(
        filtered.credentialSubject.contact.phoneNumber.extension,
      ).toBeUndefined();
      expect(
        filtered.credentialSubject.contact.phoneNumber.lineType,
      ).toBeUndefined();
      expect(
        filtered.credentialSubject.contact.phoneNumber.number,
      ).toBeUndefined();
    },
  );
  ...
  it("Should create IdDocument-v3 credential", async () => {
    ...
    const dateOfExpiry = await Claim.create(
      "claim-cvc:Document.dateOfExpiry-v1",
      dateOfExpiryValue,
      "1",
    );
    const nationality = await Claim.create(
    ...
    const evidences = await Claim.create(
  ...
  it.skip("Should create and verify a credential with an array of clains ", async () => {
    ...
    const covidClaim = await Claim.create(
      "claim-cvc:Medical.covid19-v1",
      covidDetails,
    );

    const credential = await VC.create(
      "credential-cvc:Covid19-v3",
      "",
      null,
      credentialSubject,
      [covidClaim],
  ...
  it("Should filter claims for GenericDocumentId asking for cvc:Document:Type and return only that claim", async () => {
    const typeValue = "passport";
    const type = await Claim.create(
      "claim-cvc:Document.type-v1",
      typeValue,
  ...
  test.skip(
    "cred.verify(): with a valid cred without expirationDate, should return at least" +
      " VERIFY_LEVELS.PROOFS level",
    async () => {
    const credJSon = require('./fixtures/Cred1.json'); // eslint-disable-line
      const cred = await VC.fromJSON(credJSon);
      expect(cred).toBeDefined();
      expect(await cred.verify()).toBeGreaterThanOrEqual(
        VC.VERIFY_LEVELS.PROOFS,
      );
    },
  );
  ...
  it("Should fail verification of a VC with invalid cryptographic security", async () =>
    expect(
      VC.cryptographicallySecureVerify(invalidEmailJson),
    ).resolves.toBeFalsy());
  ...
  it("Should verify an VC with cryptographic security", async () => {
    ...
    isValid = await VC.cryptographicallySecureVerify(
      credential,
  ...
  it("Should return false if attestation or signature check fail on cryptographic verification", async () => {
    ...
    let isValid = await VC.cryptographicallySecureVerify(
      credential,
      verifyAttestationFunc,
    );
  ...
  it("cred.verify(): VERIFY_LEVELS.PROOFS without expirationDate INVALID", async () => {
    ...
    credJSon.proof.leaves[0].targetHash =
      credJSon.proof.leaves[0].targetHash.replace("a", "b");
    const cred = await VC.fromJSON(credJSon);
    expect(cred).toBeDefined();
  ...
  it("should fail verification since the leaf value is tampered", async () => {
    const credentialContents = fs.readFileSync(
      "__test__/creds/fixtures/VCWithTamperedLeafValue.json",
      "utf8",
    );
  ...
  it('should have a filled "granted" field after ".grantUsageFor()" request', async () => {
    ...
    expect(cred).toBeDefined();
    expect(cred.proof.granted).toBeNull();
    cred.proof.anchor.subject = signAttestationSubject(
      cred.proof.anchor.subject,
      XPVT1,
      XPUB1,
    );
  ...
  it("should verifyGrant() accordingly", async () => {
    ...
    const subject = signAttestationSubject(
      anchoredCred.proof.anchor.subject,
      XPVT1,
      XPUB1,
    );
    const signedCred = await VC.fromJSON(
      toValueObject(
        _.merge({}, anchoredCred, { proof: { anchor: { subject } } }),
      ),
    );
  ...
  it("should fail to verify a credential json with invalid granted token with requesterGrantVerify", async () => {
    ...
    credentialObj.proof.granted =
      "304502210085f6baceefcddefff535416df0eda6c9b8a01dcba592c599ec2c83cce7171dd802204473f5a15b3904dbf0fc309fe812fbf449948714938fb4871196d338ef38f1d1";
  ...
  it.skip("should fail the check that the anchor exists on the chain", async () => {
    const credentialContents = fs.readFileSync(
      "__test__/creds/fixtures/VCTempAnchor.json",
      "utf8",
    );
    const credentialJson = JSON.parse(credentialContents);
    const cred = await VC.fromJSON(credentialJson);
  ...
  it("should fail the check with temporary attestations faked as permanent", async () => {
    const credentialContents = fs.readFileSync(
      "__test__/creds/fixtures/CredentialAttestationFaked.json",
      "utf8",
    );
    const credentialJson = JSON.parse(credentialContents);
    const cred = await VC.fromJSON(credentialJson);
  ...
  it("Should match with one constraint", async () => {
    ...
    expect(
      cred.isMatch({
        claims: [{ path: "identity.name.givenNames", is: { $eq: "Max" } }],
      }),
    ).toBeTruthy();
  });
  ...
  it("Should match with gt constraint", async () => {
    ...
    expect(
      cred.isMatch({
        claims: [{ path: "identity.dateOfBirth.year", is: { $gt: 1900 } }],
      }),
    ).toBeTruthy();
  });
  ...
  it("Should match constraints targeting the parent properties and string deltas", async () => {
    ...
    expect(
      cred.isMatch({
        claims: [{ path: "identity.dateOfBirth", is: { $lte: "-40y" } }],
      }),
    ).toBeTruthy();
  });
  ...
  it("Should not match", async () => {
    ...
    expect(
      cred.isMatch({
        claims: [{ path: "identity.name.first", is: { $eq: "Maxime" } }],
      }),
    ).toBeFalsy();
  });
  ...
  it("Should match credential on constraints.meta", () => {
    const vcMeta = {
      ...
      version: "1",
      type: ["Credential", "credential-cvc:Email-v3"],
    };
  ...
  it("Should not match credential on constraints.meta with invalid field", () => {
    const vcMeta = {
      ...
      expirationDate: "2028-09-26T11:22:21.287Z",
      version: "1",
      type: ["Credential", "civ:Credential:CivicBasic"],
    };
  ...
  it("Should return all Credential properties for credential-cvc:GenericDocumentId-v3", async () => {
    const properties = await VC.getAllProperties(
      "credential-cvc:GenericDocumentId-v3",
    );
  ...
  it("Should throw exception on credential creation if required uca is missing", async () => {
    ...
    return expect(
      ...
      /Missing required fields to credential-cvc:IdDocument-v3/,
    );
  });
  ...
  it("Should throw exception when creating a VC from json without required claims", async () => {
    ...
    return expect(VC.fromJSON(credJSon)).rejects.toThrow();
  });
});
...
describe("Transient Credential Tests", () => {
  ...
  it("Should create an US Address Transient Credential", async () => {
    ...
    const credential = await VC.create(
      "credential-cvc:UnverifiedAddress-v3",
      "",
      null,
      credentialSubject,
      [uca],
  ...
  it("Should create an US SSN Transient Credential", async () => {
    ...
    const credential = await VC.create(
      "credential-cvc:UnverifiedSsn-v3",
      "",
      null,
      credentialSubject,
      [uca],
...
describe("Signed Verifiable Credentials", () => {
  ...
  it("Should create a verifiable credential instance", async () => {
    ...
    expect(cred.proof.merkleRootSignature.verificationMethod).toBe(
      verificationMethod,
    );
...
describe("Referenced Schemas for Verifiable Credentials", () => {
  ...
  it("Validates a schema the contains a reference", async () => {
    ...
    return expect(createCredential).rejects.toThrow(
      "Missing required fields to credential-test:IdDocument-v1",
    );
  });
});
