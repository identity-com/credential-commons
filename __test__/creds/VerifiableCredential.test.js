...
describe('Unit tests for Verifiable Credentials', () => {
  ...
  test('Dont construct Credentials with wrong version', async () => {
    const name = await Claim.create('claim-cvc:Identity.name-v1', identityName);
    const dob = await Claim.create('claim-cvc:Identity.dateOfBirth-v1', identityDateOfBirth);

    return expect(VC.create('credential-cvc:Identity-v1', uuidv4(), null, [name, dob], '2'))
      .rejects
      .toThrow(/Credential definition for credential-cvc:Identity-v1 v2 not found/);
  });
  ...
  test('New Defined Credentials', async () => {
    ...
    expect(cred)
      .toBeDefined();
    expect(cred.claim.identity.name.givenNames)
      .toBe('Max');
    expect(cred.claim.identity.name.otherNames)
      .toBe('Abc');
    expect(cred.claim.identity.name.familyNames)
      .toBe('Mustermann');
    expect(cred.claim.identity.dateOfBirth.day)
      .toBe(20);
    expect(cred.claim.identity.dateOfBirth.month)
      .toBe(3);
    expect(cred.claim.identity.dateOfBirth.year)
      .toBe(1978);
  ...
  test('New Defined Credentials return the incorrect global Credential Identifier', async () => {
    ...
    expect(cred.getGlobalIdentifier())
      .toBe('credential-credential-cvc:Identity-v1-1');
  });
  ...
  test('Filter claims from Identity Name', async () => {
    ...
    expect(filtered.claim.identity.name.givenNames)
      .toBeDefined();
    expect(filtered.claim.identity.name.otherNames)
      .not
      .toBeDefined();
    expect(filtered.claim.identity.name.familyNames)
      .not
      .toBeDefined();

    const emptyFiltered = simpleIdentity.filter([]);
    expect(emptyFiltered.claim)
      .toEqual({});
  });
  ...
  it('Should filter claims for Email asking for claim-cvc:Contact.email-v1 and return them on the filtered VC',
    async () => {
      ...
      expect(filtered.claim.contact.email.domain)
        .toBeDefined();
      expect(filtered.claim.contact.email.domain.tld)
        .toBe('oVaPsceZ4C');
      expect(filtered.claim.contact.email.domain.name)
        .toBe('UTpHKFyaaB');
      expect(filtered.claim.contact.email.username)
        .toBe('ZcMpCBQ0lE');
    });
  ...
  it('Should filter claims for Address asking for claim-cvc:Type.address-v1'
    ...
    expect(filtered.claim.identity.address)
      .toBeDefined();
    expect(filtered.claim.identity.address.country)
      .toBe('X2sEB9F9W9');
    expect(filtered.claim.identity.address.county)
      .toBe('sDlIM4Rjpo');
    expect(filtered.claim.identity.address.state)
      .toBe('ZZEOrbenrM');
    expect(filtered.claim.identity.address.street)
      .toBe('JkHgN5gdZ2');
    expect(filtered.claim.identity.address.unit)
      .toBe('fo9OmPSZNe');
    expect(filtered.claim.identity.address.city)
      .toBe('LVkRGsKqIf');
    expect(filtered.claim.identity.address.postalCode)
      .toBe('5JhmWkXBAg');
  });
  ...
  it('Should filter claims for PhoneNumber asking for credential-cvc:PhoneNumber-v1 and return the full claim',
    async () => {
      ...
      expect(filtered.claim.contact.phoneNumber)
        .toBeDefined();
      expect(filtered.claim.contact.phoneNumber.country)
        .toBe('1ApYikRwDl');
      expect(filtered.claim.contact.phoneNumber.countryCode)
        .toBe('U4drpB96Hk');
      expect(filtered.claim.contact.phoneNumber.extension)
        .toBe('sXZpZJTe4R');
      expect(filtered.claim.contact.phoneNumber.lineType)
        .toBe('OaguqgUaR7');
      expect(filtered.claim.contact.phoneNumber.number)
        .toBe('kCTGifTdom');
    });
  ...
  it('Should filter claims for GenericDocumentId asking for claim-cvc:Identity.dateOfBirth-v1 and return nothing',
    async () => {
      ...
      expect(filtered.claim.document)
        .toBeUndefined();
    });
  ...
  it('Should filter claims for GenericDocumentId asking for cvc:Document:Type and return only that claim',
    async () => {
      ...
      expect(filtered.claim.document.type)
        .toBe('passport');
    });
  ...
  it('should fail to check that signature using a bad pinned key', async (done) => {
    ...
    expect(cred.proof.anchor)
      .toBeDefined();
    expect(() => cred.verifySignature(XPUB1.replace('9', '6')))
      .toThrow();
    done();
  });
  ...
  it('should verify a granted credential json with requesterGrantVerify', async (done) => {
    ...
    expect(transmittedCred)
      .toBeDefined();
    expect(transmittedCred.granted)
      .not
      .toBeNull();
  ...
  it('Should throw exception on credential creation if required uca is missing', async () => {
    ...
    return expect(VC.create('credential-cvc:IdDocument-v1', '', null, ucas, '1'))
      .rejects
      .toThrow(/Missing required claim\(s\): claim-cvc:Document.dateOfBirth-v1/);
  });
...
describe('Signned Verifiable Credentials', () => {
  ...
  test('Should create a verifiable credential instance', async () => {
    ...
    expect(cred)
      .toBeDefined();
    expect(cred.proof.merkleRootSignature)
      .toBeDefined();
    expect(cred.verifyMerkletreeSignature(pubBase58))
      .toBeTruthy();
  });
  ...
  test('Should verify credential(data only) signature', async () => {
    ...
    expect(signerVerifier.isSignatureValid(dataOnlyCredential))
      .toBeTruthy();
  });
});
