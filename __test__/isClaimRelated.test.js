...
describe("isClaimRelated Tests", () => {
  ...
  it(
    "Should validate a claim path against UCA definitions " +
      "and VC definitions and succeed",
    async () => {
  ...
  it(
    "Should validate a claim path against UCA definitions and VC definitions and " +
      "succeed returning false for an non existent dependency",
    async () => {
  ...
  it("Should fail validation of a wrong defined uca global identifier", () =>
    expect(
      isClaimRelated(
        "document.name.givenNames",
        "claim-civ:Identity:error-1",
        "credential-cvc:GenericDocumentId-v1",
      ),
    ).rejects.toThrow(/UCA identifier does not exist/));

  it("Should fail validation of a wrong defined claim path identifier", () =>
    expect(
      isClaimRelated(
        "name.error",
        "claim-claim-cvc:Document.name-v1-1",
        "credential-cvc:GenericDocumentId-v1",
      ),
