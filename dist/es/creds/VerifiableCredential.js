...
const {
  schemaLoader
} = require('../schemas/jsonSchema'); // convert a time delta to a timestamp
...
function VerifiableCredentialBaseConstructor(identifier, issuer, expiryIn, ucas, version, evidence, signerVerifier = null) {
  this.id = uuidv4();
  this.issuer = issuer;
  ...
  this.verify = async (higherVerifyLevel, options) => {
    ...
    if (verifiedlevel === VERIFY_LEVELS.INVALID && hVerifyLevel >= VERIFY_LEVELS.PROOFS && (await this.verifyProofs())) verifiedlevel = VERIFY_LEVELS.PROOFS; // Test next level

    if (verifiedlevel === VERIFY_LEVELS.PROOFS && hVerifyLevel >= VERIFY_LEVELS.ANCHOR && this.verifyAttestation()) verifiedlevel = VERIFY_LEVELS.ANCHOR; // Test next level

    if (verifiedlevel === VERIFY_LEVELS.ANCHOR && hVerifyLevel >= VERIFY_LEVELS.GRANTED && this.verifyGrant(requestorId, requestId, keyName)) verifiedlevel = VERIFY_LEVELS.GRANTED;
...
VerifiableCredentialBaseConstructor.create = async (identifier, issuer, expiryIn, ucas, version, evidence, signerVerifier = null) => {
  // Load the schema and it's references from a source to be used for validation and defining the schema definitions
  await schemaLoader.loadSchemaFromTitle(identifier); // Load the meta schema's from a source
...
VerifiableCredentialBaseConstructor.fromJSON = async (verifiableCredentialJSON, partialPresentation = false) => {
  ...
  const newObj = await VerifiableCredentialBaseConstructor.create(verifiableCredentialJSON.identifier, verifiableCredentialJSON.issuer);
