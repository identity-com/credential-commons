...
VerifiableCredentialProxy.create = async (identifier, issuer, expiryIn, ucas, version, evidence, signerVerifier = null) => {
  // Load the schema and it's references from a source to be used for validation and defining the schema definitions
  const schema = await schemaLoader.loadSchemaFromTitle(identifier); // Wrap the old signer verifier for backwards compatibility
...
VerifiableCredentialProxy.fromJSON = async (verifiableCredentialJSON, partialPresentation = false) => {
  ...
  const newObj = await VerifiableCredentialProxy.create(verifiableCredentialJSON.identifier, verifiableCredentialJSON.issuer);
...
VerifiableCredentialProxy.cryptographicallySecureVerify = async (credential, verifyAttestationFunc, verifySignatureFunc) => {
  const vc = await VerifiableCredentialProxy.fromJSON(credential);
