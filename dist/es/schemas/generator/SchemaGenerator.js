...
const generateRandomNumberValueWithRange = definition => {
  ...
  if (definition !== null) {
    if (typeof definition.minimum !== 'undefined' && definition.minimum !== null && genRandomNumber < definition.minimum) {
      genRandomNumber = definition.minimum;
    }

    if (definition.exclusiveMinimum !== 'undefined' && definition.exclusiveMinimum !== null && genRandomNumber <= definition.exclusiveMinimum) {
      genRandomNumber = definition.exclusiveMinimum + 0.1;
    }

    if (typeof definition.maximum !== 'undefined' && definition.maximum !== null && genRandomNumber > definition.maximum) {
      genRandomNumber = definition.maximum;
    }

    if (definition.exclusiveMaximum !== 'undefined' && definition.exclusiveMaximum !== null && genRandomNumber >= definition.exclusiveMaximum) {
...
const generateRandomValueForType = (definition, includeDefinitions = false) => {
  ...
  switch (resolvedTypeName) {
    case 'String':
      return refDefinition && refDefinition.pattern ? new RandExp(refDefinition.pattern).gen() : randomString.generate(10);
    case 'Number':
