...
async function isClaimRelated(claim, uca, credential) {
  ...
  await schemaLoader.loadSchemaFromTitle(credential); // first get the UCA identifier
  ...
  await schemaLoader.loadSchemaFromTitle(ucaIdentifier); // check on the credential commons if this identifier exists

  const ucaDefinition = definitions.find(definition => definition.identifier === ucaIdentifier); // does the UCA exist?

  if (ucaDefinition) {
    const ucaProperties = await Claim.getAllProperties(ucaIdentifier); // does the claim exists in the Claim?
