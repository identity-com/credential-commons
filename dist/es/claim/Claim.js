...
const getDefinition = (identifier, version) => version ? _.find(definitions, { identifier, version }) : _.find(definitions, { identifier });
...
class Claim extends UserCollectableAttribute {
  ...
  initializeAttestableValue() {
    ...
    if (parsedAttestableValue.length === 1) {
      ...
      this.value = definition.type === 'Array' ? _.map(ucaValue, item => new Claim(definition.items.type, { attestableValue: item })) : this.value = _.includes(['null', 'undefined'], ucaValue) ? null : ucaValue;
    } else {
      const ucaValue = {};
      for (let i = 0; i < parsedAttestableValue.length; i += 1) {
        ...
        ucaValue[propertyName] = new Claim(filteredIdentifier, { attestableValue: parsedAttestableValue[i].stringValue });
      }
      this.value = ucaValue;
    }
  }
  ...
  static parseAttestableArrayValue(value) {
    ...
    const parsedArrayItems = _.map(attestableValueItems, item => Claim.parseAttestableValue({ attestableValue: item }));
    return {
