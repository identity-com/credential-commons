...
class SchemaLoader {
  constructor() {
    ...
    this.ucaCompared = []; // allowUnionTypes is required because of the anchor in the proof can be a string/object (this should be changed)
  ...
  loadPropertySchema(schema, definition, ref, property) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const csProperties = yield _this2.getCredentialSubjectProperties(schema);
  ...
  getCredentialSubjectProperties(schema) {
    var _this3 = this;

    return _asyncToGenerator(function* () {
      const schemaProperties = yield _this3.flattenCredentialSchemaProperties(schema);
  ...
  flattenCredentialSchemaProperties(schema) {
    var _this4 = this;
    ...
    return _asyncToGenerator(function* () {
      ...
      if (schema.allOf) {
        const promises = schema.allOf.map( /*#__PURE__*/function () {
          var _ref = _asyncToGenerator(function* (allOf) {
            if (allOf.$ref) {
              const refSchema = yield _this4.loadSchemaFromUri(allOf.$ref);
              const refProperties = yield _this4.flattenCredentialSchemaProperties(refSchema);
              properties = _objectSpread(_objectSpread({}, properties), refProperties);
            }

            if (allOf.properties) {
              properties = _objectSpread(_objectSpread({}, properties), allOf.properties);
            }
          });
  ...
  addDefinition(schema) {
    ...
    return _asyncToGenerator(function* () {
      if (/^credential-/.test(schema.title)) {
        yield _this5.addCredentialDefinition(schema);
      } else {
        yield _this5.addClaimDefinition(schema);
      }
    })();
  }
  /**
  ...
  addCredentialDefinition(schema) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const credentialSubjectDefinition = yield _this6.getCredentialSubjectProperties(schema);
      ...
      yield _.reduce(references, /*#__PURE__*/function () {
        var _ref2 = _asyncToGenerator(function* (promise, value) {
          yield promise;
          return _this6.loadPropertySchema(schema, definition, value.ref, value.property);
        });
      ...
      _this6.credentialDefinitions.push(definition);

      _this6.validCredentialIdentifiers.push(definition.identifier);
  ...
  shouldAddClaimDefinition(schema) {
    ...
    return _asyncToGenerator(function* () {
      if (isUCA(schema.title)) {
        ...
        if (!_this7.ucaCompared.includes(schema.title)) {
          yield _this7.loadSchemaFromTitle(transformed);
        }

        _this7.ucaCompared.push(schema.title);

        let found = false;

        _this7.definitions.some(definition => {
  ...
  addClaimDefinition(schema) {
    ...
    return _asyncToGenerator(function* () {
      const definition = {
        identifier: schema.title,
        version: getSchemaVersion(schema.title),
        type: yield _this8.findDefinitionType(schema)
      };

      if (definition.type === 'Array') {
        const subSchema = yield _this8.loadSchemaFromUri(schema.items.$ref);
        definition.items = {
      ...
      if (yield _this8.shouldAddClaimDefinition(schema)) {
        _this8.definitions.push(definition);

        _this8.validIdentifiers.push(schema.title);
      }

      if (isUCA(schema.title)) {
        _this8.ucaDefinitions.push(definition);

        _this8.validUcaIdentifiers.push(schema.title);
      }
  ...
  getPropertyValue(defProperties, property, name) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (type === 'array' || items && items.$ref) {
        if (items.$ref) {
          const arraySchema = yield _this9.loadSchemaFromUri(items.$ref);
      ...
      if (property.allOf) {
        const schema = yield _this9.loadSchemaFromUri(property.allOf[0].$ref);
        type = schema.title;
      }
  ...
  getPropertyValues(properties) {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _.reduce(properties, /*#__PURE__*/function () {
        var _ref3 = _asyncToGenerator(function* (promise, value, name) {
          yield promise;
          return _this10.getPropertyValue(defProperties, value, name);
        });
  ...
  findDefinitionType(schema) {
    ...
    return _asyncToGenerator(function* () {
      if (schema.allOf) {
        const subSchema = yield _this11.loadSchemaFromUri(schema.allOf[0].$ref);
      ...
      if (schema.type === 'object') {
        if (!_.isEmpty(schema.properties)) {
          return _this11.getPropertyValues(schema.properties);
        }
      }
  ...
  loadSchemaFromTitle(title) {
    ...
    return _asyncToGenerator(function* () {
      const loader = _this12.findSchemaLoader(title);
      ...
      const existingSchema = _this12.ajv.getSchema(schemaId);
        ...
        traverse(schema, {
          cb: currentNode => {
            if (currentNode.$ref !== undefined && !currentNode.$ref.startsWith('#')) {
              ...
              references.push(_this12.loadSchemaFromUri(currentNode.$ref));
            }
          }
        });
        yield Promise.all(references);
        yield _this12.addDefinition(schema);

        try {
          _this12.ajv.addSchema(schema);
  ...
  validateSchema(identifier, data) {
    ...
    return _asyncToGenerator(function* () {
      const loader = _this13.findSchemaLoader(identifier);

      yield _this13.loadSchemaFromTitle(identifier);

      _this13.validate(loader.schemaId(identifier), data);
    })();
  }
