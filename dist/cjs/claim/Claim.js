...
const findDefinition = (identifier, version) => version ? _.find(definitions, {
  identifier,
  version
}) : _.find(definitions, {
  identifier
...
const getDefinition = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (identifier, version) {
    yield schemaLoader.loadSchemaFromTitle(identifier);
    return findDefinition(identifier, version);
  });

  return function getDefinition(_x, _x2) {
...
function adaptIdentifierIfNeeded(_x3, _x4) {
  return _adaptIdentifierIfNeeded.apply(this, arguments);
}

function _adaptIdentifierIfNeeded() {
  _adaptIdentifierIfNeeded = _asyncToGenerator(function* (identifier, version) {
    const definition = yield getDefinition(identifier, version);
    const resolvedIdentifier = definition && definition.alias ? definition.type : identifier;
    const {
      isNewIdentifier,
      identifierComponents
    } = getBaseIdentifiers(resolvedIdentifier);
    const compDefinition = yield getDefinition(resolvedIdentifier, version);

    if (!isNewIdentifier && !compDefinition) {
      const newIdentifier = `claim-cvc:${identifierComponents[1]}.${identifierComponents[2]}-v1`;
      yield schemaLoader.loadSchemaFromTitle(newIdentifier);
...
class Claim extends UserCollectableAttribute {
  ...
  static create(identifier, value, version) {
    return _asyncToGenerator(function* () {
      const currentIdentifier = yield adaptIdentifierIfNeeded(identifier, version);

      if (!value.attestableValue) {
        yield schemaLoader.validateSchema(currentIdentifier, value);
      } // Load the schema and it's references from a source to be used for validation and defining the schema definitions


      yield schemaLoader.loadSchemaFromTitle(currentIdentifier);
  ...
  initializeAttestableValue() {
    ...
    const definition = findDefinition(this.identifier, this.version);
    const parsedAttestableValue = Claim.parseAttestableValue(value); // Trying to construct UCA with a existing attestableValue
    ...
    if (parsedAttestableValue.length === 1) {
      ...
      this.value = definition.type === 'Array' ? _.map(ucaValue, item => new Claim(definition.items.type, {
        attestableValue: item
      ...
      for (let i = 0; i < parsedAttestableValue.length; i += 1) {
        const {
          propertyName
        } = parsedAttestableValue[i]; // we have stored only the property name on the urn, so we have to find the UCA definition
        ...
        if (ucaDef) {
          ...
          const splitPropertyName = propertyName.split('.'); // this property is used to check if the recursion tree has more than an depth
  ...
  static parseAttestableArrayValue(value) {
    ...
    const attestableValueItems = value.attestableValue.substring(value.attestableValue.indexOf('[') + 1, value.attestableValue.indexOf(']') - 1).split(',');

    const parsedArrayItems = _.map(attestableValueItems, item => Claim.parseAttestableValue({
      attestableValue: item
    }));
  ...
  getAttestableValue(path, isArrayItem = false) {
    ...
    const {
      identifierComponents
    } = getBaseIdentifiers(this.identifier);
    ...
    if (this.type === 'Array') {
      const itemsValues = _.reduce(this.value, (result, item) => `${result}${item.getAttestableValue(null, true)},`, '');
  ...
  static getAllProperties(identifier, pathName) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const typeDefinition = _.isString(type) ? _.find(definitions, {
        identifier: type
      }) : definition;

      if (typeDefinition && _this.getTypeName(typeDefinition) === 'Object') {
        let typeDefProps;
        ...
        let basePropName;
        const {
          identifierComponents: baseIdentifierComponents
        } = getBaseIdentifiers(identifier);
        ...
        if (_.includes(['String', 'Number', 'Boolean'], `${typeDefProps.type}`)) {
          ...
          const proProperties = yield typeDefProps.reduce( /*#__PURE__*/function () {
            var _ref2 = _asyncToGenerator(function* (prev, prop) {
              const prevProps = yield prev;
              const {
                isNewIdentifier
              } = getBaseIdentifiers(prop.type);
              const newBasePropName = !isNewIdentifier ? basePropName : `${basePropName}.${prop.name}`;
              const props = yield _this.getAllProperties(prop.type, newBasePropName);
        ...
        const {
          identifierComponents
        } = getBaseIdentifiers(definition.identifier);
        let propertiesName;
