 ...
 */
let cryptographicallySecureVerify = (() => {
  var _ref = _asyncToGenerator(function* (credential, verifyAttestationFunc, verifySignatureFunc) {
  ...
  return function cryptographicallySecureVerify(_x, _x2, _x3) {
...
function VerifiableCredentialBaseConstructor(identifier, issuer, expiryIn, ucas, version, evidence) {
  ...
  this.verify = (higherVerifyLevel, options) => {
    ...
    // Test next level
    if (verifiedlevel === VERIFY_LEVELS.INVALID && hVerifyLevel >= VERIFY_LEVELS.PROOFS && this.verifyProofs()) verifiedlevel = VERIFY_LEVELS.PROOFS;

    // Test next level
    if (verifiedlevel === VERIFY_LEVELS.PROOFS && hVerifyLevel >= VERIFY_LEVELS.ANCHOR && this.verifyAttestation()) verifiedlevel = VERIFY_LEVELS.ANCHOR;
