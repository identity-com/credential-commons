...
function DummyAnchorServiceImpl(config, http) {
  ...
  this.verifySignature = (proof, pinnedPubKey) => {
    ...
    const anchorSubjectValidation = this.verifySubjectSignature(
      subject,
      pinnedPubKey,
    );
  ...
  this.verifySubjectSignature = (subject, pinnedPubKey) => {
    ...
    const subjectSignature = ECSignature.fromDER(
      Buffer.from(subject.signature, "hex"),
    );
    return HDNode.fromBase58(pinnedPubKey || subject.pub).keyPair.verify(
