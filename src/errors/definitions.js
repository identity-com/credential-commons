...
const ErrorCodes = {
   ...
   */
  ERROR_IDV_VALIDATION_MISSING_DOC_TYPE:
    "error.idv.validation.missing.doc.type",

  /**
   * Reason: The IDV detected that the requirements fail.
   * Troubleshooting: Make sure the provided document do fit the requirements.
   */
  ERROR_IDV_VALIDATION_REQUIREMENTS_FAIL:
    "error.idv.validation.requirements.fail",
   ...
   */
  ERROR_IDV_VALIDATION_UCA_VALUE_MISMATCH:
    "error.idv.validation.uca.value.mismatch",
  ...
  ERROR_IDV_CREDENTIAL_INVALID_SIGNATURE:
    "error.idv.credential.invalid.signature",
   ...
   */
  ERROR_IDV_CREDENTIAL_FAILED_ANCHORING:
    "error.idv.credential.failed.anchoring",
  ...
  ERROR_IDV_UCA_UPDATE_NO_PROCESS_STATUS:
    "error.idv.uca.update.no.process.status",

  /**
   * Reason: A token is received before one is issued
   * Troubleshooting: Try again
   */
  ERROR_IDV_TOKEN_RECEIVED_BEFORE_ISSUE:
    "error.idv.token.received.before.issue",
