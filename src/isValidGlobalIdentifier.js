...
async function isValidGlobalIdentifier(identifier) {
  ...
  switch (splited[0]) {
    case "claim":
      if (
        !_.includes(validUCAIdentifiers, splited[1]) &&
        !_.includes(validClaimIdentifiers, identifier)
      ) {
    ...
    case "credential":
      if (
        !_.includes(validVCIdentifiers, splited[1]) &&
        !_.includes(validVCIdentifiers, identifier)
      ) {
        throw new Error(`${identifier} is not valid`);
      }
      return true;
